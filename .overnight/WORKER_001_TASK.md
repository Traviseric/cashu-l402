...
TASKS: C:\code\te-btc\cashu-l402\.overnight\active
LOG FILE: C:\code\te-btc\cashu-l402\.overnight\LOG_WORKER.md

## Step 0: Understand the Project (Read Before Coding)

Before touching code, read `C:\code\te-btc\cashu-l402\.overnight/researcher_output.json` if it exists. It tells you:
...
Read your session file: `C:\code\te-btc\cashu-l402\.overnight\active\.worker_001_session.json`
...
1. **Read the task file** from `C:\code\te-btc\cashu-l402\.overnight\active/`
...
7. **Write per-task marker** — Write `DONE` to `C:\code\te-btc\cashu-l402\.overnight/TASK_{lane:03d}_{N}_DONE`
   where N is the task number in your session (1, 2, 3, ...)
8. **Log progress** — Append to C:\code\te-btc\cashu-l402\.overnight\LOG_WORKER.md

Then move to the next task in the session.

## Feature Awareness

If `C:\code\te-btc\cashu-l402\.overnight/progress.json` contains a `features` array, check it before starting work.
...
Append to C:\code\te-btc\cashu-l402\.overnight\LOG_WORKER.md after EACH task:
...
Write to: C:\code\te-btc\cashu-l402\.overnight\worker_001_output.json
...
2. **Write the task file** to `C:\code\te-btc\cashu-l402\.overnight\active/` using the next available number:
...
1. **Read** `C:\code\te-btc\cashu-l402\.overnight\HUMAN_TASKS.md` to find the next HT-XXX ID (or start at HT-001)
