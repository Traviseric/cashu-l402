...
Check `C:\code\cashu-l402\.overnight/lessons.json` if it exists. This file accumulates knowledge from previous
...
Read `C:\code\cashu-l402/TASK_INDEX.json` if it exists. This is the universal registry of all
...
3. Read the file at the entry's `path` (relative to `C:\code\cashu-l402`)
...
Scan `C:\code\cashu-l402\.overnight` for ALL files matching `*_output.json` — these are audit results.
...
1. **`C:\code\cashu-l402/OVERNIGHT_TASKS.md`** — Master task list (if exists)
...
2. **`C:\code\cashu-l402\.overnight/active/*.md`** — Pre-existing task files (if any)
...
Write to: C:\code\cashu-l402\.overnight/reports/audit_review.json
```json
{
  "total_findings": 25,
  "accepted": 12,
...
Append rejected findings to `C:\code\cashu-l402\.overnight/lessons.json` so future sessions don't re-report them.
...
Create task files in `C:\code\cashu-l402\.overnight\active/` with this format:
...
2. **Log to `C:\code\cashu-l402\.overnight/HUMAN_TASKS.md`** — append each human-required item:
   ```markdown
...
Write to: C:\code\cashu-l402\.overnight\task_synthesizer_output.json
...
```json
{
  ...
  "next_box": "CONDUCTOR",
  "context": {
    "task_files": ["C:\code\cashu-l402\.overnight\active/001-P0-fix-issue.md", ...],
    "task_count": 25,
